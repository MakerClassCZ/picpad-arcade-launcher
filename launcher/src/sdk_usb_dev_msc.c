...
// Vendored from PicoLibSDK _sdk/usb_src/TODO/ — original relative paths
// replaced with our launcher include chain (= ../include.h pulls in
// global.h via INCLUDES_H + the rest of the SDK headers) so we don't need
// to modify the PicoLibSDK install.
#include "../include.h"
...
// Forward declaration for our stub (defined in usb_msc_screen.cpp).
// PicoLibSDK never defined this — TODO file expects it but SDK doesn't ship.
extern void UsbXferComplete(u8 epinx, u16 len, u8 xres);
...
u16 MscdOpen(const sUsbDescItf* itf, u16 max_len)
{
	...
	// PicoLibSDK UsbDevOpenEpPair drops the transfer_type arg present in the
	// original DcdOpenEpPair (= TinyUSB-style). Bulk is implied by descriptor.
	if (!UsbDevOpenEpPair(USB_DESC_NEXT(itf), 2, &MscdItf.ep_out, &MscdItf.ep_in)) return 0;
