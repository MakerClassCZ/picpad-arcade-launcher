...
// Patched by picopad_arcade: forward decls + signature adapters for MSC
// class driver (vendored in launcher/src/sdk_usb_dev_msc.{c,h}). MscdCtrlReq
// takes (stage, setup_pkt) but sUsbDevDrv expects (stage); MscdXferComp
// returns Bool but sUsbDevDrv.comp returns void — wrap both.
#if USE_USB_DEV_MSC
extern sUsbSetupPkt UsbSetupRequest;
extern void MscdInit(void);
extern void MscdTerm(void);
extern void MscdReset(void);
extern u16  MscdOpen(const sUsbDescItf* itf, u16 max_len);
extern Bool MscdCtrlReq(u8 stage, const sUsbSetupPkt* setup);
extern Bool MscdXferComp(u8 epinx, u8 xres, u16 len);
static Bool MscdCtrlReqAdapter(u8 stage)
    { return MscdCtrlReq(stage, &UsbSetupRequest); }
static void MscdXferCompAdapter(u8 epinx, u8 xres, u16 len)
...
// Patched by picopad_arcade: uncommented + corrected names for MSC source
// in _sdk/usb_*/TODO/. Without this, the dispatcher never opens the MSC
// interface, so a USB host doesn't see it as Mass Storage.
#if USE_USB_DEV_MSC	// use USB MSC Mass Storage Class (device)
	{
		MscdInit,		// initialize class driver
		MscdTerm,		// terminate class driver
		MscdReset,		// reset class driver
		MscdOpen,		// open device class interface
		MscdCtrlReqAdapter,	// process control transfer complete (signature adapter)
		MscdXferCompAdapter,	// process data transfer complete (signature adapter)
		NULL,			// receiving SOF (start of frame; NULL=not used)
		NULL,			// schedule driver
	},
#endif
